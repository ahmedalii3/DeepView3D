...
class CoordConv2D(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        x_coords = tf.reshape(x_coords, (1, 1, width, 1))
        x_coords = tf.tile(x_coords, [batch_size, height, 1, 1])

        y_coords = tf.linspace(-1.0, 1.0, height)
        y_coords = tf.reshape(y_coords, (1, height, 1, 1))
        y_coords = tf.tile(y_coords, [batch_size, 1, width, 1])

        x_coords = tf.cast(x_coords, inputs.dtype)
        y_coords = tf.cast(y_coords, inputs.dtype)

        coords = [inputs, x_coords, y_coords]
        if self.with_r:
            rr = tf.sqrt(tf.square(x_coords) + tf.square(y_coords))
            coords.append(rr)

        return tf.concat(coords, axis=-1)
...
class Residual_Unet_3D_5K_CoordConv(tf.keras.Model):
    def __init__(self,trainable = True, dtype=None, **kwargs):
        super(Residual_Unet_3D_5K_CoordConv, self).__init__()

        self.coordconv = CoordConv2D(with_r=False)
