...
class Residual_Unet_3D_5K_FeatureExtractor(tf.keras.Model):
    def __init__(self,trainable = True, dtype=None, **kwargs):
        super(Residual_Unet_3D_5K_FeatureExtractor, self).__init__()

        backbone = MobileNetV2(include_top=False, weights="imagenet", input_shape=(256, 480, 3))
        feature_layer = backbone.get_layer("block_6_expand_relu").output
        self.feature_extractor = tf.keras.Model(backbone.input, feature_layer, name="mobilenetv2_feature_extractor")
        self.feature_extractor.trainable = False
        self.feature_proj = tf.keras.layers.Conv2D(32, 1, padding="same", activation="relu")
    ...
    def call(self, inputs, training=None):
        ...
        moving_image = tf.cast(moving_image, tf.keras.backend.floatx())
        fixed_image = tf.cast(fixed_image, tf.keras.backend.floatx())

        moving_rgb = tf.reverse(moving_image, axis=[-1])
        fixed_rgb = tf.reverse(fixed_image, axis=[-1])
        backbone_training = training if self.feature_extractor.trainable else False
        moving_features = self.feature_extractor(mobilenet_preprocess(moving_rgb), training=backbone_training)
        fixed_features = self.feature_extractor(mobilenet_preprocess(fixed_rgb), training=backbone_training)

        target_size = tf.shape(moving_image)[1:3]
        moving_features = tf.image.resize(moving_features, target_size, method="bilinear")
        fixed_features = tf.image.resize(fixed_features, target_size, method="bilinear")

        moving_features = self.feature_proj(moving_features)
        fixed_features = self.feature_proj(fixed_features)

        x = tf.concat([moving_image, fixed_image, moving_features, fixed_features], axis=-1)
